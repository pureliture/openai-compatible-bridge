...
reload는 vertex 모듈의 클래스 객체(VertexAPIError, VertexChatClient,
VertexEmbeddingClient 등)를 새로 만들기 때문에, 이미 `from vertex import ...`로
그 심볼들을 바인딩해 둔 app 모듈의 참조가 stale해진다. 그 결과 app의
`except VertexAPIError`가 reload 후 raise된 새 VertexAPIError를 잡지 못하는
정체성 불일치가 발생할 수 있다.

아래 autouse 픽스처는 각 테스트가 끝난 뒤 vertex가 reload되어 app의 참조와
...
import app as _app
import vertex as _vertex

# app이 `from vertex import ...`로 가져오는 심볼 이름들.
_REBOUND_SYMBOLS = (
    "VertexAPIError",
    "VertexChatClient",
